...
"""
        result = parse_wp_config(php)
        assert result.get("DB_NAME") is None
...
class TestDiscoverWpConfig:
    ...
    def test_discover_root_path_does_not_become_empty(self):
        paths_tried = []
        ...
        result = discover_wp_config("/", FakeConn())
        assert result == "/wp-config.php"
        assert "" not in paths_tried, "remote_path='/' must not produce an empty candidate"
